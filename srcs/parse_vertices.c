...
void	*parse_vertices(char **tab, int tab_size, t_scene *scene, size_t line_nb){
	float	weight = 1.0;
	if (tab_size < 4){
		dprintf(2,"%s on line %zu is missing a float value\n", tab[0], line_nb);
		return (0x0);
	}
	if (tab[4]){
		weight = atof(tab[4]);
...
void	*parse_vertex_normals(char **tab, int tab_size, t_scene *scene, size_t line_nb){
	...
	if (!scene->vertex_normals_current){
		...
		if (!scene->vertex_normals_current->next)
			return (0x0);
...
void	*parse_texture_coordinates(char **tab, int tab_size, t_scene *scene, size_t line_nb){
	...
	if (!scene->texture_coordinates_current){
		...
		if (!scene->texture_coordinates_current->next)
			return (0x0);
